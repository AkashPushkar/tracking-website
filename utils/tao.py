...
class Tao:
    ...
    def get_first_annotation(self, track_id):
        """Return first annotation for a track.

        Useful for, e.g., user-initialized tracking."""
        return min(self.track_ann_map[track_id],
                   key=lambda x: self.imgs[x['image_id']]['frame_index'])
